...
public class TlsOnlySocketFactory extends SSLSocketFactory {
	...
	public TlsOnlySocketFactory() {
		this.delegate = SSLCertificateSocketFactory.getDefault(
	...
	public Socket createSocket(String host, int port, InetAddress localHost,
		...
		return makeSocketSafe(
				delegate.createSocket(host, port, localHost, localPort), host);
	}
	...
	private class TlsOnlySSLSocket extends DelegateSSLSocket {
		...
		private TlsOnlySSLSocket(SSLSocket delegate, boolean compatible) {
			...
			if (compatible) {
				...
				super.setEnabledCipherSuites(enabled.toArray(new String[enabled
						.size()]));
				return;
			} // else
			...
			ArrayList<String> protocols = new ArrayList<String>(
					Arrays.asList(delegate.getSupportedProtocols()));
			protocols.remove("SSLv2");
			protocols.remove("SSLv3");
			super.setEnabledProtocols(protocols.toArray(new String[protocols
					.size()]));
			...
			Pattern exclude = Pattern
					.compile(".*(_DES|DH_|DSS|EXPORT|MD5|NULL|RC4).*");
			for (String cipher : delegate.getSupportedCipherSuites()) {
				if (!exclude.matcher(cipher).matches()) {
					enabledCiphers.add(cipher);
				}
			}
			super.setEnabledCipherSuites(enabledCiphers
		...
		public void setEnabledProtocols(String[] protocols) {
			if (protocols != null && protocols.length == 1
				...
				if (this.compatible) {
					systemProtocols = Arrays.asList(delegate
							.getEnabledProtocols());
				} else {
					systemProtocols = Arrays.asList(delegate
				...
				if (enabledProtocols.size() > 1) {
					...
					LogUtil.e(TAG, "SSL stuck with protocol available for "
	...
	public class DelegateSSLSocket extends SSLSocket {
		...
		// inspired by
		// https://github.com/k9mail/k-9/commit/54f9fd36a77423a55f63fbf9b1bcea055a239768

		public DelegateSSLSocket setHostname(String host) {
			try {
				delegate.getClass().getMethod("setHostname", String.class)
